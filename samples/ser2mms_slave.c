...
#if defined(LINUX)&&defined(ARM) 
  pps_t pps = pps_init("", 0, 1000000000, 500000000);
  if (!pps) printf("[main] pps=null\r\n");
  
  if (!pps_enable(pps, true)) {
    printf("[main] can't enable\r\n");
  }
#endif
...
/*           // Параметр 0
          ...
          if ( SET_ATTR(f32, p, IEDMODEL_UPG_GGIO1_Id_mag_f, ftemp) ) {
            //SET_ATTR(t, X, IEDMODEL_UPG_GGIO1_Id_t, carg_buf[0].pul);
            SET_ATTR(q, p, IEDMODEL_UPG_GGIO1_Id_q, true);
          }
          // Параметр 1
          ftemp = (f32_t)carg_buf[1];
          if ( SET_ATTR(f32, p, IEDMODEL_UPG_GGIO1_Ud_mag_f, ftemp) ) {
            //SET_ATTR(t, X, IEDMODEL_UPG_GGIO1_Ud_t, carg_buf[1].pul);
            SET_ATTR(q, p, IEDMODEL_UPG_GGIO1_Ud_q, true);
          }
          // Параметр 2
          ftemp = (f32_t)carg_buf[2];
          if ( SET_ATTR(f32, p, IEDMODEL_UPG_GGIO1_Iz_mag_f, ftemp) ) {
            //SET_ATTR(t, X, IEDMODEL_UPG_GGIO1_Iz_t, carg_buf[2].pul);
            SET_ATTR(q, p, IEDMODEL_UPG_GGIO1_Iz_q, true);
          } */
        } break;
