                  ...
                  void *pld_api, u32_t mode, u32_t id, void *stty_init)
{
  ...
  _self_is_used = 1;
  struct transp_s *self = &_self;
  memset((void*)self, 0, sizeof(struct transp_s));
#else
  struct transp_s *self = calloc(1, sizeof(struct transp_s));
  if (!self) return NULL;
#endif
...
/**
  * @brief Receive next byte wrapper
  * @param argv - ?
  */
...
/**
  * @brief Transmit next byte wrapper
  * @param argv - ?
  */
...
static void tick(void *opaque)
{
  ...
  {  
    case RECV_INIT: {
      //ev_post( self->ev_rcvd, EV_RDY ); //TODO зачем это?
    } break;
